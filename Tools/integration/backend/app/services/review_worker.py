...
async def process_review_job(
    ...
    job=None,                   # the existing Job ORM object
) -> None:
    """
    ...
    try:
        # ── Core review — the sealed service boundary ─────────────────────────
        ...
        job.result_id = str(review.id)
        job.completed_at = datetime.utcnow()
        await db.commit()

        logger.info(
            "review_job_completed job_id=%s result_id=%s score=%d depth=%s findings=%d",
            job_id, review.id, review.overall_score or 0,
            review.depth_level, len(report.findings),
        )

    except ReviewError as exc:
        # Known failure — store structured error, do not re-raise
        ...
        # Unexpected — store generic error, re-raise for worker-level logging
        review = Review.from_error(
            job_id=job_id, repo_url=repo_url, branch=branch,
