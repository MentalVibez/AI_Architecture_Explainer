...
Separation of concerns:
    Job  = queue lifecycle (queued → running → completed/failed)
...
class Review(Base):
    ...
    # ── Engine metadata (queryable, explains score differences) ───────────────
    ruleset_version:  Mapped[str] = mapped_column(String(20), nullable=True)
    ...
    # ── Score scalars (queryable without JSON operators) ──────────────────────
    ...
    operations_score:           Mapped[int]  = mapped_column(Integer, nullable=True)
    developer_experience_score: Mapped[int]  = mapped_column(Integer, nullable=True)
    ...
    # ── Full report payloads (JSONB for indexing in Postgres) ─────────────────
    ...
    # ── Failure tracking ──────────────────────────────────────────────────────
    # Set on both failed reviews AND as diagnostic context on completed ones
    ...
    @classmethod
    def from_error(
        cls, job_id: uuid.UUID, repo_url: str,
